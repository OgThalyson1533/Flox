...
export function syncActiveViewLabel(index = 0) {
  ...
  // Tabs 5-11 não têm botão próprio → "Mais" deve ficar ativo para todas elas
  const isMoreTab = index >= 5 && index <= 11;
  if (dot && maisBtn) {
    if (isMoreTab) {
...
export function switchTab(index, { force = false, skipHistory = false } = {}) {
  const target = clamp(Number(index) || 0, 0, 11);
  ...
  const legacyTabToBtn = { 0:0, 1:3, 2:1, 3:3, 4:2, 5:3, 6:3, 7:3, 8:3, 9:3, 10:3, 11:3 };
  ...
  if (target === 11) requestAnimationFrame(renderBanks);
}
